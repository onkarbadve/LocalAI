...
flowchart LR
    ...
    subgraph Fedora["🖥️ Fedora box — daily driver"]
        ...
        QwenU["llama-server: Qwen3-4B abliterated<br/>:8081 (production, mutually exclusive)"]
        GemmaU["llama-server: Gemma-4-E4B abliterated<br/>:8082 (production, mutually exclusive)"]
    ...
    QwenU -->|Vulkan| GPU["Intel Iris Xe iGPU"]
    GemmaU -->|Vulkan| GPU
```

**Updated 2026-08-05**: the third node, `llama-server: Qwen3-4B` (censored, port 8080, "production"), was removed from this diagram — that model and its launch script (`start-qwen3.sh`) were deleted, along with the censored Gemma vision model and `start-gemma-e4b.sh`. Only uncensored models are kept on this box now, and the two remaining `llama-server` instances above are themselves production, not a "blunt/direct" secondary tier next to a censored primary.
...
- **`llama-server` (×2 registered, 1 running at a time)** — the inference engine, built from `llama.cpp` source with the Vulkan backend. An uncensored/blunt-mode Qwen3 variant (`start-qwen3-uncensored.sh`, port `8081`) and an uncensored/blunt-mode Gemma variant (`start-gemma-uncensored.sh`, port `8082`) — both production, both register as connections in Open WebUI, but only one process runs at once — combined Vulkan memory doesn't fit more than one simultaneously at their current context sizes (see each script's own memory-budget comment, and [SETUP.md](../SETUP.md#qwen3-4b-instruct-2507-heretic-av2--uncensored-bluntdirect-assistant-start-qwen3-uncensoredsh-fedora-only)). Whichever port is live just shows up in Open WebUI's model picker. A third, censored Qwen3 instance (`start-qwen3.sh`, port `8080`) used to also be registered here — deleted 2026-08-05 along with its model file, since only uncensored models are kept on this box now.
...
- **OpenVINO Model Server (evaluated, not pictured above)** — a second inference backend evaluated as an alternative to llama.cpp/Vulkan, Podman-based. Qwen3-8B (`start-ovms-qwen3-8b.sh`, port 8084) is deliberately not wired into Open WebUI and not part of the daily-driver architecture above. Qwen3.5-9B goes one step further: its text instance (`start-ovms-qwen3.5-9b-text.sh`, port 8085, GPU) *is* registered as a real Open WebUI connection — a second chat option, not a replacement for the production llama.cpp models — while its vision counterpart (`start-ovms-qwen3.5-9b-vision.sh`, port 8086, CPU-pinned) exists to route image requests around a GPU-only hallucination bug ([openvino#37223](https://github.com/openvinotoolkit/openvino/issues/37223)) and was never actually launched. See [SETUP.md](../SETUP.md#alternative-backend-openvino--ovms-evaluated-not-in-daily-use) and [JOURNAL.md](../JOURNAL.md) (2026-08-02/03/04).

## Design decisions worth calling out

- **Two chat models registered, one running**: rather than reconfigure Open WebUI's connection every time the active model changes, both `llama-server` ports are registered as connections up front (`OPENAI_API_BASE_URLS`, semicolon-separated). Switching models is just restarting the relevant `start-*.sh` script — Open WebUI reconnects automatically. (Was three ports before 2026-08-05; see the Components note above.)
