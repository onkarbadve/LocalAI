...
| Qwen3.5-9B (int4-ov), text-only | GPU (iGPU), via OVMS (`start-ovms-qwen3.5-9b-text.sh`) | not separately re-measured — same IR/device as the `openvino_genai` row above | — | ~18s warm restart (`--cache_dir` on a named volume), ~26–43s cold | 2026-08-04 |
