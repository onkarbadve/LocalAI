...
- `pi-config/` - reproducible, secret-free system config for the Raspberry Pi (`raspberrypi-pihole`) - Quadlet units, nftables, Caddy, watchdog/arbiter scripts, and a `provision.sh` to rebuild a fresh box's *shape*. The Pi itself still isn't in this repo (see [docs/overall-setup.md](docs/overall-setup.md)) - only its config is tracked here; secrets and accumulated state live only in the backup archive, see [docs/pi-backup-restore.md](docs/pi-backup-restore.md) and [pi-config/README.md](pi-config/README.md).
