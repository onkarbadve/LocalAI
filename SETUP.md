...
- `OPENAI_API_BASE_URLS="http://localhost:8080/v1;http://localhost:8081/v1;http://localhost:8082/v1"`, `OPENAI_API_KEYS="sk-no-key-required;sk-no-key-required;sk-no-key-required"` (plural, semicolon-separated — added 2026-07-30, extended 2026-08-01 with port 8082): registers the production Qwen3 server (8080), the uncensored Qwen3 server (8081, `start-qwen3-uncensored.sh`), and the uncensored Gemma server (8082, `start-gemma-uncensored.sh`) as connections at once, so whichever one is actually running shows up in the model picker without reconfiguring Open WebUI each time. The three `llama-server` processes are still run mutually exclusively (combined Vulkan memory doesn't fit more than one, see JOURNAL.md 2026-07-30) — whichever port is down just appears unreachable in the picker rather than erroring at Open WebUI startup.
- `ENABLE_RAG_WEB_SEARCH=true` / `RAG_WEB_SEARCH_ENGINE=searxng` / `SEARXNG_QUERY_URL` (added 2026-08-01): wires the chat "Web Search" toggle to the self-hosted SearXNG instance (`start-searxng.sh`, port 8888) instead of an external search API. Requires `start-searxng.sh` running first.
- `ENABLE_KB_EXEC=true`: gives Native-mode models a filesystem-style interface (ls/tree/grep/cat) over attached Knowledge, in addition to search tools.
...
UI: <http://localhost:3000> — first visit creates the local admin account. Switching which llama.cpp model is active just means restarting the underlying `start-*.sh` script for the port you want live; Open WebUI already has 8080, 8081, and 8082 registered as connections (see above), so it reconnects automatically rather than needing reconfiguration.
...
### Container management — Quadlet + auto-update (Fedora only)

All 7 Podman containers on this box (`open-webui`, `searxng`, `open-terminal`, `metube`, `ovms-qwen3-8b`, `ovms-qwen3.5-9b-text`, `ovms-qwen3.5-9b-vision`) are defined as systemd Quadlet units in `~/.config/containers/systemd/*.container`, generated into `~/.config/systemd/user/*.service` on `systemctl --user daemon-reload`. Set up 2026-08-14 — see JOURNAL.md that date for the full rationale/rollout.

- **Why Quadlet over the old hand-rolled `podman run -d` scripts**: the old `start-*.sh` pattern checked `podman container exists` and did `podman start -a` on a hit — reusing the *existing* container even after a fresh `podman pull`, so an image update silently didn't take effect until someone manually noticed and did `podman stop && podman rm`. Quadlet's generated service always runs a fresh `podman run --replace`, so the container is recreated from the current image every start. This was found and fixed live during a real OVMS image update — see JOURNAL.md 2026-08-14.
- **`start-*.sh` scripts still work** — each is now a thin wrapper (`systemctl --user is-active` check + `systemctl --user start <name>.service`), kept so the documented `./start-X.sh` invocations above don't change. Direct `systemctl --user start/stop/restart/status <name>` also works and is arguably more idiomatic now.
- **Auto-update policy is split per service**, set via the `AutoUpdate=` key in each `.container` file:
  - `registry` (open-webui, searxng, open-terminal, metube): `podman-auto-update.timer` pulls the registry's current tag and recreates the container if it changed — no local `podman pull` needed first.
  - `local` (all three `ovms-*`): the timer only recreates against whatever image is *already* pulled locally — it never pulls on its own. Deliberate: OVMS's GPU-serving containers use a main-branch `weekly` dev tag (or `latest-gpu`) chosen specifically for runtime freshness against known GPU-kernel bugs (openvino#36151) — a blind background pull could regress that without anyone checking logs for `CL_OUT_OF_RESOURCES` first. `podman pull` + a manual verification pass (see `start-ovms-qwen3.5-9b-text.sh`'s header) stays a deliberate action.
- **None of the 7 units are `[Install]`-enabled** (no `WantedBy=`), matching the pre-Quadlet behavior — nothing auto-starts at boot/login even though `loginctl enable-linger onkar` is set (needed for rootless user units to survive logout and for the timer to fire at all). Containers still only come up when their `start-*.sh` script (or `systemctl --user start`) is run. This is also why the three `ovms-*` GPU-users are safe to leave defined but stopped — the timer skips a unit that isn't currently running (nothing to recreate), so an idle OVMS server won't get silently started by the update timer.
- **Timer**: `podman-auto-update.timer`, enabled via `systemctl --user enable --now podman-auto-update.timer` — runs on systemd's stock daily schedule (`systemctl --user list-timers podman-auto-update.timer` shows the next run). Check what it did: `journalctl --user -u podman-auto-update.service`. Dry-run without changing anything: `podman auto-update --dry-run`.
- **Image pruning**: `podman auto-update` never removes the old image layers it replaces, so a custom `~/.config/systemd/user/podman-image-prune.{service,timer}` (daily 01:00, an hour after the auto-update run) runs `podman image prune -af` — safe, only removes layers no running container references. Added 2026-08-14 alongside the auto-update rollout after noticing this box's `/home` was at 94% full (3.4G free of 54G) — worth checking `df -h /home` periodically even with this in place, since it only reclaims orphaned image layers, not `models/` or other real data growth.
- **Open Terminal's API key** doesn't fit Quadlet's `EnvironmentFile=` (which needs `KEY=VALUE` lines, not a raw secret) — `start-open-terminal.sh` now also mirrors `~/.config/open-terminal/api-key` into `~/.config/open-terminal/api-key.env` (`OPEN_TERMINAL_API_KEY=...`, `chmod 600`) on every run, so key rotation + rerunning the script still just works.
